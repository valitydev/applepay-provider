...
import dev.vality.provider.applepay.domain.PaymentToken;
...
import static dev.vality.provider.applepay.service.DecryptionTool.*;
...
public class DecryptionService {
    ...
    public String decryptToken(String merchantId, PaymentToken paymentToken) throws CryptoException {
                ...
                if (merchantCertificate != null) {
                    if (certificateMatchesKeyAndMerchantId(merchantCertificate, keyHash, merchantId)) {
                        return DecryptionTool.decrypt(paymentToken.getPaymentData().getHeader().getEphemeralPublicKey(),
                                Base64.getDecoder().decode(paymentToken.getPaymentData().getData()), pkcs12Data,
                                merchantCertificate, pksc12KeyPass);
                    }
                }
            }
        } catch (Exception e) {
        ...
        try {
            return DecryptionTool.decrypt(paymentToken.getPaymentData().getHeader().getEphemeralPublicKey(),
                    Base64.getDecoder().decode(paymentToken.getPaymentData().getData()), merchCertData, pkcs12KeyData,
                    pksc12KeyPass);
    ...
    public boolean certificateMatchesKeyAndMerchantId(X509Certificate merchantCertificate, String keyHash,
                                                      String merchantId) throws NoSuchAlgorithmException {
        return (pubKeyHash(merchantCertificate).equals(keyHash) && merchantId.equals(
                getMerchantUID(merchantCertificate)));
    }
}
