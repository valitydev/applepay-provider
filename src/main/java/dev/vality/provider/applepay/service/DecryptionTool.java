...
public class DecryptionTool {
    ...
    static {
        ...
        KDF_OTHER_BYTES_PREFIX = ((char) 0x0D + "id-aes256-GCM" + "Apple").getBytes(StandardCharsets.US_ASCII);

    }

    public static String decrypt(String ephemeralPublicKeyData, byte[] tokenData, byte[] merchantCerData,
                                 byte[] pkcs12KeyData, char[] pkcs12KeyPass)
            throws IOException, CertificateException, KeyStoreException, NoSuchAlgorithmException, UnrecoverableEntryException, NoSuchProviderException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, NoSuchPaddingException {
        // read the ephemeral public key. It's a PEM file without header/footer -- add it back to make our lives easy
        String ephemeralPubKeyStr =
        ...
        final X509Certificate merchantCertificate =
                readDerEncodedX509Certificate(new ByteArrayInputStream(merchantCerData));
    ...
    public static String decrypt(String ephemeralPublicKeyData, byte[] tokenData, byte[] pkcs12Data,
                                 X509Certificate merchantCertificate, char[] pkcs12KeyPass)
            throws IOException, CertificateException, KeyStoreException, NoSuchAlgorithmException, UnrecoverableEntryException, NoSuchProviderException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, NoSuchPaddingException {
        // read the ephemeral public key. It's a PEM file without header/footer -- add it back to make our lives easy
        String ephemeralPubKeyStr =
                "-----BEGIN PUBLIC KEY-----\n" + ephemeralPublicKeyData + "\n-----END PUBLIC KEY-----";
    ...
    public static X509Certificate getCertificate(byte[] pkcs12Data, char[] pkcs12KeyPass, File filename)
    ...
    private static byte[] decrypt(byte[] cipherText, ECPrivateKey merchantPrivateKey, ECPublicKey ephemeralPublicKey,
    ...
    private static ECPrivateKey loadPrivateKey(byte[] pkcs12Data, char[] pass)
    ...
    private static byte[] extractMerchantIdentifier(X509Certificate merchantCertificate)
        ...
        return hexStringToByteArray(new String(merchantIdentifier, StandardCharsets.US_ASCII));
    }

    private static X509Certificate readDerEncodedX509Certificate(InputStream in)
