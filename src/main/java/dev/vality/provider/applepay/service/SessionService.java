...
public class SessionService {
    ...
    public String requestSession(String merchantId, String validationURL, String body)
            throws CryptoException, APSessionException, WRuntimeException {
    ...
    private OkHttpClient prepareClient(byte[] identityCert, char[] pass) throws CryptoException {
        return new OkHttpClient.Builder().connectTimeout(connTimeoutMs, TimeUnit.MILLISECONDS)
                .writeTimeout(writeTimeoutMs, TimeUnit.MILLISECONDS).readTimeout(readTimeoutMs, TimeUnit.MILLISECONDS)
                .addInterceptor(new HttpLoggingInterceptor())
                .sslSocketFactory(sslProvider.getSSLForClient(identityCert, pass), sslProvider.getX509TrustManager())
                .dns(dnsSelector).build();
    }

    private Request prepareRequest(String validationURL, String body) {
        RequestBody requestBody =
                RequestBody.create(MediaType.parse("Content-type: application/json;charset=utf-8"), body);
