...
import dev.vality.provider.applepay.domain.PaymentHeader;
import dev.vality.provider.applepay.domain.ValidationException;
...
public class SignatureValidator {
    ...
    private static void validateCustomData(ArrayList<X509CertificateHolder> allCertificates)
            throws ValidationException {
        if (allCertificates.size() != 2) {
            throw new ValidationException(
                    "Signature certificates count expected 2, but it's :" + allCertificates.size());
        }
    ...
    private static void validateECCSignature(SignerInformation signerInformation, Store store)
            throws ValidationException {
        try {
    ...
    private static void validateSignatureTime(long applePaySignatureExpirationInMs, SignerInformation signerInformation)
        ...
        if (signDate == 0) {
            throw new ValidationException(
                    "Failed to extract sign time from apple pay signature [ASN1UTCTime not found].");
        }
        long expiration = System.currentTimeMillis() - applePaySignatureExpirationInMs;
        if (expiration > signDate) {
            throw new ValidationException(
                    String.format("Apple pay signature is too old: %s, the expiration time is: %s ms.", signDate,
                            applePaySignatureExpirationInMs));
        }
    }
    ...
    private static void validateCertificate(X509Certificate leafCertificate, X509Certificate trustedRootCert,
        ...
        try {
            ...
            CertStore intermediateAPCertStore =
    ...
    public void validate(PaymentHeader applePayHeader, String applePayData, String applePaySignature, String version)
            throws ValidationException {
        try {
            boolean eccSign = isECC(version);

            byte[] signedData = eccSign ? getECCSignedData(applePayData, applePayHeader)
                    : getRSASignedData(applePayData, applePayHeader);
            ...
            ArrayList<X509CertificateHolder> allCertificates =
                    (ArrayList<X509CertificateHolder>) store.getMatches(null);
            SignerInformation signerInformation = cmsSignedData.getSignerInfos().getSigners().stream().findFirst()
                    .orElseThrow(() -> new ValidationException("Cannot extract siner info"));
