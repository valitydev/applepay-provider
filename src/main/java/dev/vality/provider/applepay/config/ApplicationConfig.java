...
public class ApplicationConfig {
    ...
    @Bean
    public APCertStore certStore(@Value("${cert.base:@null}") String baseDir,
                                 @Value("${cert.identity.path:@null}") String identityDir,
                                 @Value("${cert.processing.path:@null}") String processingDir) {
    ...
    @Bean
    public SessionService sessionService(APCertStore certStore, @Value("${cert.identity.pass}") String identityPass,
                                         @Value("${apple.conn_timeout}") int connTimeoutMs,
                                         @Value("${apple.read_timeout}") int readTimeoutMs,
                                         @Value("${apple.write_timeout}") int writeTimeoutMs, DnsSelector dnsSelector) {
        return new SessionService(certStore, identityPass.toCharArray(), connTimeoutMs, readTimeoutMs, writeTimeoutMs,
                dnsSelector);
    }

    @Bean
    public SignatureValidator signatureValidator(@Value("${cert.ca.path}") Resource resource,
                                                 @Value("${apple.expiration_time}") Long expirationTime)
            throws IOException {
        return new SignatureValidator(Files.readAllBytes(resource.getFile().toPath()), expirationTime);
    }

    @Bean
    public DecryptionService decryptionService(APCertStore apCertStore,
                                               @Value("${cert.processing.pass}") char[] keyPass) {
        return new DecryptionService(apCertStore, keyPass);
    }

    @Bean
    public PaymentToolProviderSrv.Iface providerHandler(SignatureValidator validator,
                                                        DecryptionService decryptionService,
                                                        @Value("${apple.validation}") Boolean validation) {
    ...
    public FilterRegistrationBean externalPortRestrictingFilter() {
        Filter filter = new OncePerRequestFilter() {
            ...
            protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
                ...
                if ((request.getLocalPort() == restPort || request.getLocalPort() == managementPort)
