...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT,
        properties = {"cert.identity.path=./target/test-classes/apple_keys/identity",
                "cert.processing.path=./target/test-classes/apple_keys/processing", "apple.expiration_time=-1"})
...
public class DecryptionRunnerTest {
    ...
    public void setUp() throws URISyntaxException {
        client = new THSpawnClientBuilder().withNetworkTimeout(5000).withAddress(new URI(thriftUrl))
    ...
    public void testDecryption() throws IOException, TException {
        ...
        ApplePayRequest payRequest = new ApplePayRequest("merchant.money.rbk.checkout",
                new Content("application/json", ByteBuffer.wrap(paymentToken)));
    ...
    public void testMissingFields() throws TException {
        byte[] paymentToken =
                "{\"token\":{\"paymentMethod\":{\"displayName\":\"Simulated Instrument\",\"network\":\"MasterCard\"},\"transactionIdentifier\":\"Simulated Identifier\"}}".getBytes();
        ApplePayRequest payRequest = new ApplePayRequest("merchant.money.rbk.checkout",
                new Content("application/json", ByteBuffer.wrap(paymentToken)));
        WrappedPaymentTool wrapped = new WrappedPaymentTool(PaymentRequest.apple(payRequest));
        UnwrappedPaymentTool unwrapped = client.unwrap(wrapped);
